...
import { PaginatedBody, TimeBasedStatsBody } from "../types/response";
import {
  PrimaryQuery,
  PromoteUserRequest,
  UpdateUserRequest,
...
class UserController {
  ...
  async initializeAdminUser(): Promise<void> {
    try {
      ...
      if (!adminExists && adminRole) {
        ...
        logger.info(`Admin ${adminUser.email} đã được khởi tạo thành công.`);
      }
    } catch (error) {
      logger.error("Lỗi khi khởi tạo admin user", error);
    }
  }

  // lấy dữ liệu của bản thân
  async getMyInfo(req: AuthRequest, res: Response, next: NextFunction) {
    const userId = req.id;
  ...
  async deleteUser(
    ...
    try {
      ...
      if (borrowRecord) {
        throw new AppError(
          "Không thể xóa người này vì người này có sách chưa trả",
          400,
          "/users",
          "DELETE"
        );
